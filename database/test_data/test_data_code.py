...
while restart_count < 5 and has_errors:
    ...
    try:
        if not -6324043907266028338 in succeeded_hashes:  # avoid duplicate inserts
            instance = airport1 = Airport(name="John F. Kennedy International Airport", city="New York", country="USA", iata_code="JFK", icao_code="KJFK")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-6324043907266028338)
    except Exception as e:
        has_errors = True
    ...
    try:
        if not 8705886363043765710 in succeeded_hashes:  # avoid duplicate inserts
            instance = airport2 = Airport(name="Los Angeles International Airport", city="Los Angeles", country="USA", iata_code="LAX", icao_code="KLAX")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(8705886363043765710)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 241295794191111867 in succeeded_hashes:  # avoid duplicate inserts
            instance = airport3 = Airport(name="Heathrow Airport", city="London", country="UK", iata_code="LHR", icao_code="EGLL")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(241295794191111867)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not -2943636730133334290 in succeeded_hashes:  # avoid duplicate inserts
            instance = airport4 = Airport(name="Changi Airport", city="Singapore", country="Singapore", iata_code="SIN", icao_code="WSSS")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-2943636730133334290)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
